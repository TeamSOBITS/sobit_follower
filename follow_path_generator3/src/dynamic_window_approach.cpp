...
namespace follow_path_generator3 {
    ...
    bool DynamicWindowApproach::predictPath( const double vel, const double ang_vel, const DWAParameters& dwa_param, EvaluatedPath* paths, MinMaxValue* mmv ) {
        ...
        Path path, pre_path; 
        tmp_path.x_vel = vel; tmp_path.ang_vel = ang_vel;
    ...
    DynamicWindowApproach::DynamicWindowApproach ( ) : nh_() {
        ...
        fp_.reset ( new person_tracking3::FollowPosition() );
    }
    ...
    bool DynamicWindowApproach::generatePath2Target ( const geometry_msgs::Twist& curt_vel, const person_tracking3::FollowPosition& fp, geometry_msgs::Twist* output_path ) {
        ...
        *fp_ = fp;
        ...
        for ( auto& pl : paths_list ) {
            if ( pl.score.is_collision || ( pl.x_vel == 0.0 && pl.ang_vel == 0.0 ) ) continue;
