...
#include "hsrb_library/hsrb_joint_controller.hpp"
...
namespace multiple_sensor_person_tracking {
    class HSRbPersonAimSensorRotator : public rclcpp::Node {
        private:
			...
			std::unique_ptr<hsrb::HSRbJointController> hsrb_ctr_;
        ...
        public:
            explicit HSRbPersonAimSensorRotator(const rclcpp::NodeOptions & options)
            : rclcpp::Node("hsrb_person_aim_sensor_rotator", options),
...
void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::callbackData (
	...
	if ( use_rotate_ ) hsrb_ctr_->moveHeadPanTilt ( pan_angle, tilt_angle, sec, false );
...
void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::onInit() {
	...
	hsrb_ctr_ std::make_unique<hsrb::HSRbJointController>();
    ...
    sub_following_position_ = this->create_subscription<FollowingPosition>(
        following_position_topic_name, 1, std::bind(&HSRbPersonAimSensorRotator::callbackData, this, std::placeholders::_1));

	if ( !use_rotate_ ) return;
	hsrb_ctr_->moveToPose( "initial_pose" );
	hsrb_ctr_->moveHeadPanTilt ( 0.0, 0.2, 0.3, false );
}

RCLCPP_COMPONENTS_REGISTER_NODE(multiple_sensor_person_tracking::HSRbPersonAimSensorRotator)
