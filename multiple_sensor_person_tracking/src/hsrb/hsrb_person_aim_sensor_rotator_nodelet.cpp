...
namespace multiple_sensor_person_tracking {
    class HSRbPersonAimSensorRotator : public nodelet::Nodelet {
        private:
			ros::NodeHandle nh_;
			ros::NodeHandle pnh_;
...
void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::makeMarker( const double pan_angle, const double tilt_angle, const double distance ) {
...
void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::callbackDynamicReconfigure(multiple_sensor_person_tracking::SensorRotatorParameterConfig& config, uint32_t level) {
...
void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::callbackData (
    const multiple_sensor_person_tracking::FollowingPositionConstPtr &following_position_msg
    /*const nav_msgs::OdometryConstPtr &odom_msg*/) {
	...
	double sec = ( distance == 0.0 ) ? 1.5 : 0.15;

	tilt_angle = 0.24001132304441541;
	...
	// if ( use_rotate_ ) hsrb_ctr->moveHeadPanTilt ( pan_angle, tilt_angle, 3, true );
	if ( display_marker_ ) makeMarker( pan_angle, tilt_angle, distance );

	return;
}

void multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::onInit() {
    nh_ = getNodeHandle();
	...
	// hsrb_ctr.reset( new sobit_pro::SobitProJointController );
    ...
    f_ = boost::bind(&multiple_sensor_person_tracking::HSRbPersonAimSensorRotator::callbackDynamicReconfigure, this, _1, _2);
    ...
    // sync_->registerCallback ( boost::bind( &HSRbPersonAimSensorRotator::callbackData, this, _1, _2 ) );

    sub_following_position_ = nh_.subscribe(pnh_.param<std::string>( "following_position_topic_name", "/following_position" ), 1, &HSRbPersonAimSensorRotator::callbackData, this);


	if ( !use_rotate_ ) return;
	// hsrb_ctr->moveToPose( "initial_pose" );
	// hsrb_ctr->moveHeadPanTilt ( 0.0, 0.0, 0.0, false );
}

PLUGINLIB_EXPORT_CLASS(multiple_sensor_person_tracking::HSRbPersonAimSensorRotator, nodelet::Nodelet);
