...
ParticlePersonTracker::ParticlePersonTracker() : nh_(), pnh_("~")  {
    ...
    pub_sensor_rotator_ = nh_.advertise<person_tracking4::RotatePosition>("rotate_position", 1);
}

int ParticlePersonTracker::findPersonsPosition( const sobit_common_msg::ObjectPoseArrayConstPtr object_pose_array, std::vector<geometry_msgs::Point>* person_pts ) {
    int person_num = 0;
    std::vector<geometry_msgs::Point> pts;
    for ( const auto &pose : object_pose_array->object_poses ) {
        if( pose.Class != "person" ) continue;
        pts.push_back( pose.pose.position );
        person_num++;
    }
    *person_pts = pts;
    return person_num;
    ...
    person_tracking4::FollowPositionPtr follow_position ) 
{
    ...
    if( exists_tgt ) fph_.addObstacle( cloud_obs );
    ...
    *follow_position = fph_.getFollowPosition ( );
    ...
    person_tracking4::RotatePosition rotate_pos;
    rotate_pos.process_flag = follow_position->process_flag;
    rotate_pos.target_distance = follow_position->target_distance;
    rotate_pos.target_angle = follow_position->target_angle;
    pub_sensor_rotator_.publish(rotate_pos);
    // Update Time :
    curt_time_ = ros::Time::now().toSec();
    //  Allocate memory :
    fph_.initFollowPosition();
