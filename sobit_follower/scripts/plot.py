...
fig, ax = plt.subplots()
print("\tDistance amd Velocity")
ax.set_xlabel('t')  # x軸ラベル
ax.set_ylabel('Distance[m] & Velocity[m/s]')  # y軸ラベル
ax.set_title('Distance amd Velocity') # グラフタイトル
# ax.set_aspect('equal', adjustable='box') # スケールを揃える
ax.grid()            # 罫線
#ax.set_xlim([-10, 10]) # x方向の描画範囲を指定
#ax.set_ylim([0, 1])    # y方向の描画範囲を指定
ax.plot(time_d, distance, color="blue", label="Distance")
ax.plot(time_v, linear, color="red", label="Translational")
