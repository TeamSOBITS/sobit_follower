...
namespace person_following_control {
    ...
    class SobitEduPersonFollowing : public nodelet::Nodelet {
        private:
            ros::NodeHandle nh_;
            ros::NodeHandle pnh_;
...
void person_following_control::SobitEduPersonFollowing::callbackDynamicReconfigure(person_following_control::PersonFollowingParameterConfig& config, uint32_t level) {
...
void person_following_control::SobitEduPersonFollowing::virtualSpringModelDynamicWindowApproach(
    const multiple_sensor_person_tracking::FollowingPositionConstPtr &following_position_msg,
    const nav_msgs::OdometryConstPtr &odom_msg,
    geometry_msgs::TwistPtr output_path)
{
...
void person_following_control::SobitEduPersonFollowing::virtualSpringModel (
    const multiple_sensor_person_tracking::FollowingPositionConstPtr &following_position_msg,
    const nav_msgs::OdometryConstPtr &odom_msg,
    geometry_msgs::TwistPtr output_path)
{
...
void person_following_control::SobitEduPersonFollowing::dynamicWindowApproach (
    const multiple_sensor_person_tracking::FollowingPositionConstPtr &following_position_msg,
    const nav_msgs::OdometryConstPtr &odom_msg,
    geometry_msgs::TwistPtr output_path)
{
...
void person_following_control::SobitEduPersonFollowing::rotatePID (
    const multiple_sensor_person_tracking::FollowingPositionConstPtr &following_position_msg,
    const nav_msgs::OdometryConstPtr &odom_msg,
    geometry_msgs::TwistPtr output_path)
{
...
void person_following_control::SobitEduPersonFollowing::obstacles_callback (const sensor_msgs::PointCloud2ConstPtr &obstacles_msg)
{
    obstacles_msg_ = obstacles_msg;
}

void person_following_control::SobitEduPersonFollowing::onInit() {
    nh_ = getNodeHandle();
    ...
    sub_obstacles_ = nh_.subscribe(pnh_.param<std::string>( "obstacles_topic_name", "/obstacles"), 1, &SobitEduPersonFollowing::obstacles_callback, this);
    ...
    sync_->registerCallback ( boost::bind( &SobitEduPersonFollowing::callbackData, this, _1, _2 ) );
    // dynamic_reconfigure :
    server_ = new dynamic_reconfigure::Server<person_following_control::PersonFollowingParameterConfig>(pnh_);
    f_ = boost::bind(&SobitEduPersonFollowing::callbackDynamicReconfigure, this, _1, _2);
    server_->setCallback(f_);
    use_pid_ = false;
    pre_time_ = ros::Time::now().toSec();
}

PLUGINLIB_EXPORT_CLASS(person_following_control::SobitEduPersonFollowing, nodelet::Nodelet);
