...
#include <sobit_follower2/FollowPosition.h>
...
class GroupedPointsArray2FollowPosition {
	private :
		bool convertTargetMsg( const geometry_msgs::Point& input, geometry_msgs::Point* target, double* distance, double* angle );
		bool convertObstacleMsg ( const std::vector<sobit_follower::grouped_points>& input, std::vector<sobit_follower2::Obstacle>* obstacles );

	public :
		GroupedPointsArray2FollowPosition();
		void convertMsg ( const sobit_follower::grouped_points_arrayPtr input, sobit_follower2::FollowPosition* output );
};

GroupedPointsArray2FollowPosition::GroupedPointsArray2FollowPosition () {

}
void GroupedPointsArray2FollowPosition::convertMsg ( 
	const sobit_follower::grouped_points_arrayPtr input, 
	sobit_follower2::FollowPosition* output ) {
	sobit_follower2::FollowPosition fp;
	fp.exists_target = convertTargetMsg ( input->target_point, &fp.target, &fp.target_distance, &fp.target_angle );
	fp.is_nearby_obstacle = convertObstacleMsg ( input->grouped_points_array, &fp.obstacles );
	*output = fp;
}
bool GroupedPointsArray2FollowPosition::convertTargetMsg ( 
	const geometry_msgs::Point& input, 
	geometry_msgs::Point* target, 
	double* distance, 
	double* angle ) {
    *target = input; 
    *distance = std::hypotf( input.x, input.y);
    *angle = std::atan2 ( input.y, input.x );
	return true;
}
bool GroupedPointsArray2FollowPosition::convertObstacleMsg ( 
	const std::vector<sobit_follower::grouped_points>& input,
	std::vector<sobit_follower2::Obstacle>* obstacles ) {
	std::vector<sobit_follower2::Obstacle> tmp_obss;
	bool is_nearby_obstacle = false;
	for ( auto& obs : input ) {
		sobit_follower2::Obstacle tmp_obs;
		tmp_obs.center.x = obs.center_x;
		tmp_obs.center.y = obs.center_y;
		double dist = std::hypotf( tmp_obs.center.x, tmp_obs.center.y );
        if( dist < 0.35 ) is_nearby_obstacle = true; 
		int pt_size = obs.particle_x.size();
		for( int i = 0; i < pt_size; i++ ) {
			geometry_msgs::Point tmp_pt;
			tmp_pt.x = obs.particle_x[i];
			tmp_pt.y = obs.particle_y[i];
			tmp_obs.points.push_back( tmp_pt );
		}
		tmp_obss.push_back(tmp_obs);
	}
	*obstacles = tmp_obss;
	return is_nearby_obstacle;
}
...
class target_scan_class
{
	...
	ros::Publisher pub_follow_position;
	...
	GroupedPointsArray2FollowPosition gpa2fp_;
...
public:
    ...
    ros::param::get( "obstacle_point_topic_name", obstacle_point_topic_name );
		...
		this->pub_follow_position = nh.advertise<sobit_follower2::FollowPosition>("/follow_position", 1 );
		...
		else//ターゲットの追跡用
		{
			...
			if( min_obstacle_distance_from_last_target_point > distance_from_last_target_point_limit )//前のtargetの位置から離れすぎている場合
			{
				...
				if(lost_target_count > wait_lost_count)
				{
					...
					gpa2fp_.convertMsg( input, &fp );
					this->pub_obstacles_states.publish(input);
					this->pub_follow_position.publish(fp);
				}//if
