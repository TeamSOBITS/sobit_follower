...
fmt_name = "sample_kalman_{0:%Y年%m月%d日-%H時%M分}.txt".format(now)
...
with open(D, mode='a') as f:
	...
	if __name__ == '__main__':
		rospy.loginfo('ok')
		rospy.init_node('kalman_point_listener', anonymous=True)
		rospy.Subscriber("/kalman_point",Point, callback)
