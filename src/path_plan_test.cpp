...
void dwa()
{
	...
	for(double ang_velo = min_ang_velo; ang_velo < max_ang_velo; ang_velo += delta_ang_velo)//for_2
	{
		for(double velo = min_velo; velo < max_velo; velo += delta_velo)//for_3
		{
			...
			predict_state(ang_velo,velo,current_robot_x,current_robot_y,current_robot_ang,samplingtime,pre_step);
...
void predict_state(double ang_velo,double velo,double x,double y,double th,double dt,double st)	//pathを予測する
{
	for(int i = 0; i < st; i++)
	{
		double temp_x = velo * cos(th) * dt + x;
		double temp_y = velo * sin(th) * dt + y;
		double temp_th = ang_velo * dt + th;
		this->ph.next_x = temp_x;
		this->ph.next_y = temp_y;
		this->ph.next_th = temp_th;
...
void evaluate()
{
	...
	for( int i = 0; i < this->obstacle_states.grouped_points_array.size(); i ++ )//for_1
	{
		...
		double distance_from_center_pre_point = hypotf(this->ph.next_x - center_point_x , this->ph.next_y - center_point_y);
		for(int j = 0; j < this->obstacle_states.grouped_points_array[i].particle_x.size(); j++)//for_2
		{
			...
			double distance_from_ob_point_pre_point = hypotf(this->ph.next_x - obstacle_point_x , this->ph.next_y - obstacle_point_y);
	...
	if(this->es.collision == true) {return;}//以下の処理は行わない
	this->es.obs_.score = min_obs_distance_score;//score評価1::障害物との距離

	double posi_from_pre_to_goal_x = this->base_laser_point.point.x - this->ph.next_x;
	double posi_from_pre_to_goal_y = this->base_laser_point.point.y - this->ph.next_y;

	double frame_from_pre_to_goal_x = posi_from_pre_to_goal_x * cos(this->ph.next_th) - posi_from_pre_to_goal_y * sin(this->ph.next_th);
	double frame_from_pre_to_goal_y = posi_from_pre_to_goal_x * sin(this->ph.next_th) + posi_from_pre_to_goal_y * cos(this->ph.next_th);
	...
	this->es.vel_.score = fabs(this->ph.canditate_velo);//score評価4::車輪の前進速度がどの程度かを判定
	this->path_eval_list.push_back(this->es);//push

	//各評価値の最大値と最小値を算出(正直書き方が下手)
